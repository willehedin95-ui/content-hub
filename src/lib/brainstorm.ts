...
- **VISUAL CONTEXT MUST signal our niche, not the competitor's.** Even if the competitor's image is a generic UGC scene (person at laptop, person in mirror, neutral lifestyle moment) without their product visible, your image_prompts MUST integrate visual elements that signal what WE sell. Read the PRODUCT KNOWLEDGE section above carefully — extract the core noun(s) of our niche (e.g. puppy/dog for dog training, pillow/bed for sleep aids, skin/face for skincare, supplement bottle for ingestible products) and ensure those nouns appear visually in EVERY prompt's Subject or Arrangement field. A doginwork ad without a visible puppy fails because the viewer scrolls past wondering what we sell. Adapt the SCENE TYPE (e.g. "person reacting emotionally in front of laptop") into our niche ("person reacting emotionally with their puppy beside them"). Never preserve the source's literal scene if it omits our niche's visual anchor.
...
- **TEXT IN IMAGES — CRITICAL**: You MUST analyze whether the competitor ad has text overlays or not.
  ...
  - **If the competitor ad has text that is physically PART OF THE SCENE** (handwritten on body/skin, written on paper/sign, tattoo-style text, marker text, text on a product label, text on a mirror, whiteboard, Venn diagram, social media UI mockup like a Facebook/Instagram comment card, Twitter/X post, iMessage/SMS screenshot, Trustpilot/Google review card, chat bubble overlay, screenshot mockup, etc.): you MUST describe the ADAPTED text for our product directly in the Nano Banana prompt${USE_JSON_PROMPTING ? ' (in the "Subject", "RoomObjects", or "Arrangement" key of the JSON)' : ''}. Example: if a beauty supplement competitor has "Drains the cortisol face" written on skin, your prompt must say something like "handwritten text on her upper arm reading 'Stopped snoring after one night on this pillow' in the same casual marker style. **NO OTHER TEXT appears anywhere in the scene — no subtitle, no second line, no caption, no tagline, no supporting copy. The single text element is the only writing visible in the image.**". In this case, set hook_text and headline_text to empty strings since the text is already in the prompt, BUT set \`"has_text": true\` so the image gets translated to other languages.
  - **CRITICAL — UI mockups ARE the persuasion mechanism, never strip them.** When the source ad uses a social-media UI mockup (Facebook comment card, Instagram post, Twitter/X post, iMessage screenshot, Trustpilot review card, etc.) as a scene element, that mockup IS the ad's persuasion structure — usually a fake-looking-real testimonial. You MUST adapt the mockup with OUR product's testimonial copy and keep it in the scene. NEVER drop it on the assumption that "UGC adaptation means just the photo". Reproduce the mockup faithfully (same UI chrome — profile circle, name, comment text, like/reply/share/timestamp footer) but rewrite the comment to fit our niche and target avatar. Example: if source has "My husband was crying like I was cutting onions. This song is everything." in a Facebook comment card, your prompt's Arrangement field must describe a similar Facebook-style comment card overlay in the lower portion of the image with the adapted Swedish testimonial inside (e.g. "Min man trodde jag var galen när jag grät. Marie's kurs räddade vår familj."). The mockup card stays; only the comment content adapts.
  - **CRITICAL anti-hallucination guard**: Whenever text appears in the scene (handwritten sign, whiteboard, written label, etc.), your prompt MUST explicitly state that NO OTHER TEXT exists in the image. Without this guard, Nano Banana will hallucinate "supporting" subtitles, USP-style second lines, or tagline-like copy under the main text — pulling vocabulary from product context (claims, benefits, USPs in PRODUCT KNOWLEDGE). The model treats writable surfaces as marketing real-estate and fills empty space with extra copy unless told otherwise. Mirror the source ad's text DENSITY exactly: if competitor's sign has one line, your prompt must enforce one line.
...

{
  ...
  "concept": {
    ...
    "ad_copy_primary": ["1 primary ad text (${langLabel(generationLanguage ?? "en")}). ADAPT from the competitor's copy structure, tone, and length — reproduce their hook style, agitation pattern, proof types, and CTA. Match their word count. **CRITICAL FORMAT**: the JSON string value MUST contain literal \\\\n\\\\n (backslash-n-backslash-n, JSON-encoded blank line) between every distinct thought. Example shape: \\"Hook line.\\\\n\\\\nProblem agitation.\\\\n\\\\nSolution reveal.\\\\n\\\\nProof point.\\\\n\\\\nCTA.\\" — note the \\\\n\\\\n between every paragraph. Single-paragraph wall-of-text strings are REJECTED. One sentence per paragraph. Must read as a structural adaptation, not generic product copy."],
