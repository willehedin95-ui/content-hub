...
import { TrendingUp, TrendingDown, Minus, Search, MousePointerClick, Eye, Target, RefreshCw } from "lucide-react";
...
function TrendBadge({ value, suffix = "%", inverse = false }: { value: number; suffix?: string; inverse?: boolean }) {
  if (value === 0) return <span className="text-xs text-gray-400 flex items-center gap-0.5"><Minus className="w-3 h-3" /> 0{suffix}</span>;
  // For position, positive trend = good (moved up). For clicks/impressions, positive = good.
...
function KpiCard({ label, value, trend, trendSuffix, icon: Icon, trendInverse }: {
  label: string;
  value: string | number;
  trend?: number;
  trendSuffix?: string;
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-4">
      ...
      <div className="text-2xl font-semibold text-gray-900">{typeof value === "number" ? value.toLocaleString() : value}</div>
      {trend !== undefined && (
        <div className="mt-1">
          <TrendBadge value={trend} suffix={trendSuffix} inverse={trendInverse} />
    ...
    return (
      <div className="bg-white rounded-lg border border-gray-200 p-12 text-center">
        ...
        <p className="text-sm text-gray-500 mb-6 max-w-md mx-auto">
          Configure your Google Search Console properties in the Settings tab, then sync to pull in your keyword data.
        </p>
        <a href="/seo?tab=settings" className="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700">
          Go to Settings
        </a>
      </div>
    );
