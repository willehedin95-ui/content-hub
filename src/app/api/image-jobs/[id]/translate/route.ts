...
export async function POST(
  ...
  try {
    ...
    const prompt = `Recreate this exact image but translate all text from English to ${langLabel}. Keep the same visual style, layout, colors, and design. Only translate the text.`;
    ...
    return NextResponse.json({ translatedUrl: urlData.publicUrl });
  } catch (error) {
    // Mark as failed
    await db
      .from("image_translations")
