  ...
  const promptObj: Record<string, unknown> = hasSource
    ...
    : {
        ...
        subject: {
          ...
          permanent_features_lock:
            "ALL permanent skin features must be IDENTICAL between halves - moles, birthmarks, beauty marks, scars, freckle patterns, ear shape, eyebrow shape, the placement of any individual mark on the skin. These are IDENTITY markers, not 'between-days variations'. If a mole exists on the left cheek in one half, the exact same mole exists in the exact same place in the other half. Do NOT add, remove, or relocate any permanent skin feature between halves.",
          identity_lock:
            "Both halves show the SAME person on two different days - recognizably the same individual (same face structure, eye color, hair color, age, same permanent skin features). Natural between-photos variations that ARE allowed: how loose hair strands fall, head lean angle, micro-expression, transient skin appearance (slight redness, undereye puffiness, sheen). NOT allowed: any change to permanent identifying features (see permanent_features_lock). Do NOT clone the pose - these are two separate phone selfies weeks apart, not two frames from one session.",
        },
        before_half: {
          outfit: beforeTop,
        ...
        hard_constraints: [
          ...
          "PERMANENT SKIN FEATURES MUST MATCH EXACTLY: any mole, birthmark, beauty mark, scar, or distinctive freckle visible in one half must appear in the EXACT SAME PLACE in the other half, identical in size/shape/color. These are identity markers - the model must NOT invent a new mole or birthmark that only exists in one half. If a feature is visible on the right cheek of the BEFORE half, it must be visible on the right cheek of the AFTER half in the same exact position.",
