...
export async function GET(req: NextRequest) {
  ...
  // Date range: last 7 usable days (accounting for GSC delay)
  const now = new Date();
  ...
  // Build previous period position averages keyed by "query|country|property"
  ...
  // Aggregate current period by query+country+property
  ...
  for (const r of curRows) {
    ...
    if (existing) {
      ...
      existing.posSum += r.position ?? 0;
      existing.count += 1;
      // Keep the page with most impressions
  ...
  // Build result array with trends
  const results = Array.from(aggMap.entries()).map(([key, agg]) => {
    ...
    const positionTrend = prevAvgPosition !== null ? prevAvgPosition - avgPosition : 0; // positive = improved
  ...
  if (sortBy === "position") {
    ...
    results.sort((a, b) => b.positionTrend - a.positionTrend);
  } else {
