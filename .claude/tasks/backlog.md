...
- [x] ~~**Investigate HappySleep autopilot failure streak**~~ — Root cause: `swipeCompetitorAd` ran 3 image gens **sequentially** (90–270s/concept) inside a cron with `maxDuration=300`. Vercel killed the cron mid-loop, leaving partial source_images. Fixed: parallelized image gen via `Promise.allSettled` + bumped maxDuration to 800 (Vercel fluid compute max). Also recovered #156 (completed) + #153/#157 (archived). Commit `8b33087`. (done 2026-04-07)
- [x] ~~**Consider deprecating finish-and-queue**~~ — Decided to keep. finish-and-queue handles draft→full pipeline including translations (manual brainstorm path). /review approve only handles already-translated ready concepts. Not redundant. (done 2026-04-07)
- [ ] **Verify tomorrow's autopilot run actually succeeds** — After commit `8b33087`, the cron at 08:00/08:30 UTC should produce 3/3 successful concepts (not 0–1). Look for "X/3 images failed" warnings in Vercel logs which would indicate Kie rate limiting from parallelization. (added 2026-04-07, HIGH)
