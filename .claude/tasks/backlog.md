...
- [ ] **Verify pipeline-push pushes Hydro13 #12-#18** — Tomorrow's 03:00 UTC run should push the 6 newly-approved concepts. Watch for "[Pipeline Cron] Reconciled N stuck jobs" in Vercel logs to confirm reconcile is running. (added 2026-04-07, HIGH)
- [ ] **Investigate HappySleep autopilot failure streak** — #146, #151, #152, #154, #157 all stuck or failed in a row from March 29 → April 4. Almost every day. Dig through cron logs for pattern: brainstorm errors, JSON parse, Kie failures, specific competitor ad scoring issues. (added 2026-04-07, HIGH)
- [ ] **Consider deprecating finish-and-queue** — Now that it just wraps `approveConceptAction` + `triggerAutopilotTranslations`, it's almost identical to /review approve. Could be replaced entirely. (added 2026-04-07, LOW)
