...
1. **Parallelized image gen** in [src/lib/swipe-competitor.ts](content-hub/src/lib/swipe-competitor.ts) — converted sequential `for` loop to `Promise.allSettled`. Retry loop preserved per-image. Cuts 3-image gen from 90–270s to 30–90s.
2. **Bumped maxDuration** in [src/app/api/cron/autopilot-concepts/route.ts](content-hub/src/app/api/cron/autopilot-concepts/route.ts) from 300 → **800** (Vercel fluid compute max).

Build verified clean (`npx tsc --noEmit` + `npm run build`).

### 4. ✅ Decided NOT to deprecate finish-and-queue
Read [src/lib/approval-actions.ts](content-hub/src/lib/approval-actions.ts) and traced both flows:
- **finish-and-queue**: For DRAFT concepts (manual brainstorm) — promotes draft→ready, calls `approveConceptAction`, then triggers translations
- **/review approve**: For READY concepts — only does approval (no translations because draft→ready transition already triggered them upstream)
...
- **Keep finish-and-queue**: Even though it looks like a wrapper, it's the only path that drives draft→full pipeline including translations for manual brainstorm. /review approve doesn't fit because it expects already-translated concepts.
...
**Deployed:**
- Commit `8b33087` pushed to main on Vercel auto-deploy
...
1. **Verify tomorrow's autopilot run (08:00 / 08:30 UTC)** — check that the parallelization fix works in production. Look for:
   - 3/3 concepts successfully created (not 0–1)
   - All 3 source_images per concept
   - No "X/3 images failed" warnings in logs
2. **Monitor pipeline-push for Hydro13 #12-#17** — cold start should keep rolling these out over the next few days.
3. **Consider monitoring the partial-success log** I added (`[swipe-competitor] X/3 images failed for job Y`). If this pattern shows up, Kie rate limiting is the next root cause to investigate.
4. **Check `auto_paused_ads` table tomorrow morning** — confirm cleanup-empty-adsets runs daily without rate-limit issues.
